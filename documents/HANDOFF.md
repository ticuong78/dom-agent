...
**Purpose.** This document is for future coding agents (and contributors) joining the project mid-stream. It records what the engine looks like *and why*, what was decided collaboratively, and what is still pending. Read this before changing anything in `src/`; the reasoning explains design choices that the code alone cannot.
...
- Pipeline (also the order of `documents/proccesses/*.md`):
  ...
  4. **Report** — `DiffReport` bundles `DiffPoint[]`; `Renderer` turns them into a string (HTML or JSON); `DiffReporter` writes that string to disk.
...
| 4 | **Reporters that return string/buffer instead of forcing fs write** | Core OSS — via `Renderer` interface | ✅ Done (`JSONRenderer` + refactored `JSONDiffReporter`) |
...
2. **Classes own their semantics.** A class named `NodeMutationDiffViewer` must *know* what mutation detection means; it does not borrow that knowledge from external rules passed in by the caller. Naming is a contract. Avoid "empty shells waiting to be filled."
3. **Hexagonal ports stay open.** Generic types default to `string`; subclasses can narrow. Users may bring custom `DiffViewer`s, `Comparer`s, `Renderer`s, etc., without modifying the core.
...
Added in this refactor. Optional `string`, **mutable** field on `DiffPoint`. Stamped by `AbstractDiffViewer.stamp()` post-hoc; the `DiffPoint` constructor does *not* take a `source` argument.
...
> **Matched beats unmatched.** If any child viewer paired reference-node R with target-node T (via any non-`ADDED`/`DELETED` diff type), every `DELETED` emitted for R and every `ADDED` emitted for T by *other* viewers is dropped.
...
2. **Discover matched sets.** Walk `allDiffs` once. Build `matchedRefs` = every `referenceNode` appearing in a non-`ADDED` diff; `matchedTars` = every `targetNode` appearing in a non-`DELETED` diff.
3. **Filter contradictions.** Drop any `DELETED` whose `referenceNode ∈ matchedRefs`; drop any `ADDED` whose `targetNode ∈ matchedTars`.
4. **Deduplicate.** Collapse points sharing the same key `(type, referenceNode.id ?? "", targetNode.id ?? "")` — keep the first.
5. **Return.** Surviving points retain their original `source`; composite does not overwrite.
...
### 4.5 `JSONRenderer` + `JSONDiffReporter` separation

`JSONRenderer` (at `src/implementation/renderer/JSONRenderer.ts`) implements `Renderer` and does the single job of `JSON.stringify(report.serialize())`. `JSONDiffReporter` is now a thin wrapper that delegates to a `Renderer` (default `JSONRenderer`) and only owns the filesystem write.

**Why this matters for SaaS.** The SaaS needs the JSON string without touching the filesystem. It uses `Renderer` directly:
...
Bypass the reporter when there is no file to write. Same pattern applies to HTML — use `new DeepSpaceTheme().render(report)` (themes implement `Renderer`) and bypass `HTMLDiffReporter`.
...
src/
  core/
    diff/
      DiffPoint.ts             # now with source?: string
      DiffViewer.ts            # interface
      DiffReport.ts
      DiffReporter.ts          # interface
      AbstractDiffViewer.ts    # stub (kept for path compatibility; real impl in implementation/)
    compare/
      Comparer.ts              # interface — now has setCompareRule / setGroupBy
      CompareRule.ts
    renderer/Renderer.ts       # interface
  ...
  implementation/
    diff/
      viewer/
        ...
        index.ts
      reporter/
        HTMLDiffReporter.ts               # takes a Renderer
        JSONDiffReporter.ts               # takes a Renderer (default JSONRenderer)
    renderer/
      JSONRenderer.ts                     # implements Renderer
      index.ts
    compare/RuleBasedComparer.ts          # rule?: CompareRule; compare() throws if unset
    converter/HTMLToContextConverter.ts
    theme/                                # HTML themes — also implement Renderer
...
2. **`JSONRenderer` introduced.** Refactored `JSONDiffReporter` to delegate. Exported via `src/implementation/renderer/index.ts` and `src/index.ts`.
...
1. **`deserialize` / `fromSnapshot` methods** for `ContextNode`, `ContextTree`, `DiffPoint`, `DiffReport`. Required for DB round-trip; SaaS persistence depends on this.
...
2. **Refresh `documents/proccesses/*.md`.** The step docs were written before `CompositeDiffViewer`, `AbstractDiffViewer`, and `DEFAULT_RULE` existed. They need to mention these.
3. **`TODO.md` cleanup.** Item #3 (`CompositeDiffViewer`) is now done — mark it.
4. **`CHANGELOG.md` entry + version bump.** Current `package.json` says `1.1.1`. This refactor is breaking-ish (composite + default rules + source field + reporter constructors). Suggest `1.2.0` or `2.0.0` depending on how strict the project is about SemVer for breaking constructor changes (`JSONDiffReporter` constructor changed shape).
5. **README touch-up.** The "Choosing the Right Comparer" table is now redundant — each viewer owns its rule. Quick Start example should be the one-line composite usage.
