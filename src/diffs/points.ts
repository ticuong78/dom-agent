import type { ContextNodeSnapshot, ContextNode } from "@convert/context";
...
export class DiffPoint<T extends string = DiffType> implements ISerializable {
  ...
  // XXXX, do not save id of a referenceNode as id of a diffpoint
  // Instead, query them whenever user needs an element by passing down id
  // diffs[<id>]
  // id: string
...
export class DiffResult<T extends string = DiffType> {
  readonly diffPointsMap: Map<string, DiffPoint<T>[]>;

  constructor() {
    this.diffPointsMap = new Map<string, DiffPoint<T>[]>();
    ...
    return new Proxy(this, {
      get(target, property, receiver) {
        ...
        return target.getDiffPoints(property);
      },

      set(target, property, value, receiver) {
        if (value instanceof DiffPoint) {
          target.addDiffPoint(value);
          return true;
        }
  ...
  getDiffPoints(referenceId: string) {
    return this.diffPointsMap.get(referenceId);
  }

  addDiffPoint(diffPoint: DiffPoint<T>) {
    // Use only referenceNode.id as the identity for a DiffPoint.
    const lookupId = diffPoint.referenceNode?.id;

    if (!lookupId) return;

    const diffPoints = this.diffPointsMap.get(lookupId) ?? [];
    diffPoints.push(diffPoint);

    this.diffPointsMap.set(lookupId, diffPoints);
  }
}
